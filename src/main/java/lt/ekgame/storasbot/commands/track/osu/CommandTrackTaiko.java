...
@CommandReference(isGuild=true, labels = {"track-taiko", "t-t"})
public class CommandTrackTaiko extends TrackOsuBase implements Command<BotCommandContext> {
	
	@Override
	public String getHelp(CommandFlags flags) {
		return getHelp("track-taiko");
	}

	@Override
	public CommandResult execute(CommandIterator command, BotCommandContext context) {
		return handleOsuTrack(command, context, OsuMode.TAIKO);
	}
}